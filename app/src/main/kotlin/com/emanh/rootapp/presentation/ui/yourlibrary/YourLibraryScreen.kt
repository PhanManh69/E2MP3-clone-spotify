...
fun YourLibraryScreen() {
    STFHeader(userName = "emanh", type = STFHeaderType.HeaderLibrary, libraryList = sampleLibraryData, content = {
        LazyColumn(modifier = it.padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
            items(10) { index ->
                Box(modifier = Modifier
                    .fillMaxWidth()
                    .height(80.dp)
                    .background(color = if (index == 0) Color.Magenta else Color.Yellow))
            }
        }
    })
}