...
fun HomeCardImage(
    ...
    val cardTitle = limitedList.map {
        when (it) {
            is HomeAlbumsData -> it.nameAlbum
    ...
    val cardSubtitle = limitedList.map {
        when (it) {
            is HomeAlbumsData -> "${it.songs.size}, 1hr 32min"
    ...
    val cardType = limitedList.map {
        when (it) {
            is HomeAlbumsData -> R.string.album
    ...
    val cardImageUrlList = limitedList.map {
        when (it) {
            is HomeAlbumsData -> it.songs.map { song -> song.imageUrl }
    ...
    val cardDescription = limitedList.map { item ->
        when (item) {
            ...
            is HomeAlbumsData -> item.songs.map { song -> song.title }.distinct().joinToString(", ")
            else -> ""
        }
    }
